...
class CustomLoginView(LoginView):
    def get_success_url(self):
        ...
        logger.info(
            f"Użytkownik {user} przekierowany do {super().get_success_url()} po zalogowaniu."
        )
...
class CustomRegisterView(FormView):
    ...
    success_url = reverse_lazy('account:account_activation_sent')
    ...
    def form_valid(self, form):
        ...
        email.send()
        logger.info(
            f"Utworzono konto dla {user.email}. Wysłano mail aktywacyjny na domenę {current_site.domain}."
        )
        notify_admin(
            "Nowy użytkownik", f"Użytkownik {user.username}, e-mail: {user.email}"
        )
...
class ActivateAccount(View):
    def get(self, request, uidb64, token, *args, **kwargs):
        ...
        if user is not None and default_token_generator.check_token(user, token):
            ...
            logger.warning(
                f"Nieudana próba aktywacji konta: uid={uidb64}, token={token}."
            )
        ...
        booking_events = [
            {
                ...
                "title": b.event.name,
                "city": b.event.venue.city if b.event.venue else b.event.city,
                "venue": b.event.venue.name if b.event.venue else b.event.venue,
            }
            for b in booking_items
        ]

        last_3_bookings = booking_items.filter(
            event__date__lte=datetime.date.today()
        ).order_by("-event__date")[:3]

        nearest_3_bookings = booking_items.filter(
            event__date__gte=datetime.date.today()
        ).order_by("event__date")[:3]

        return render(
            request,
            "account/user_home.html",
            {
...
class UserUpdateView(LoginRequiredMixin, UpdateView):
    ...
    login_url = reverse_lazy("account:home")
    success_url = reverse_lazy("account:info")  # Redirect after saving
    ...
    def get_success_url(self):
        return reverse_lazy("account:info")
...
class CustomPasswordResetView(PasswordResetView):
    ...
    email_template_name = "account/user_reset_password_email.html"
    success_url = reverse_lazy("account:password_reset_sent")


class ResetPasswordSentView(TemplateView):
    template_name = "account/user_reset_password_done.html"
