...
from apps.users.management.commands.load_users import Command
...
def test_login_after_loading_users(loaded_users, users_from_json):    
    ...
    result_logged_in = authenticate(username=user_json["username"], password=user_json["password"])
    result_denied = authenticate(username=user_json["username"], password="wrongpassword")
