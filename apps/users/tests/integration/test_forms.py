...
def test_user_create_form_clean_email():
    ...
    User.objects.create_user(
        username="jan", email="jan@example.com", password="pass123"
    )
...
def test_user_profile_form_invalid_file():
    ...
    form = UserProfileForm(
        data={}, files={"photo": invalid_file}, instance=UserProfile(user=user)
    )

    assert not form.is_valid()  # depends on your validators
    assert "photo" in form.errors
    assert form.errors["photo"] == [
        "Prześlij poprawny plik graficzny. Aktualnie przesłany plik nie jest grafiką\xa0lub jest uszkodzony."
    ]
